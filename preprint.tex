...
\begin{document}
   ...
   \end{itemize}
	...
	We assume a dataset consisting of $n$ samples. The samples are annotated by a design matrix $X \in \mathbb{R}^{n \times q}$, where $q$ is the number of features. The intercept column is not supposed to be a part of the design matrix, as our model handles intercept terms as explicit parameters.
	...
	Our inference model relaxes one assumption of the section \nameref{dynamical_model}: we require the elongation speed to be constant only within each intron, and denote by $k_{\mathrm{elong},i}$ the elongation speed over the intron $i$. The polymerase density then remains constant within each intron, equal to $k_{\mathrm{init}} / k_{\mathrm{elong},i}$, so the results \eqref{eq:i_nas_ss}, \eqref{eq:p_nascent} and \eqref{eq:pi_rates} hold as derived, with $k_{\mathrm{elong}}$ replaced by $k_{\mathrm{elong},i}$.
	...
	rates, and thus six such ratios up to inversion. The corresponding six LFCs are, however, not independent of
	each other, as they span a space of dimensionality $3$: each of them is a difference of two of the four individual rate LFCs, and such differences span a three-dimensional space, since the four individual LFCs are determined only up to a common additive shift, which is exactly the scaling invariance \eqref{eq:scaling_invariance}. That is, our inference model needs to parametrize only three of them, with the remaining three LFCs following as their linear combinations.

	The choice of which three ratios to parametrize is a matter of convention: any other choice of three independent ratios is a linear reparametrization of the same three-dimensional space. We parametrize the three ratios that share $k_{\mathrm{deg}}$ as their denominator, and denote
	their LFCs as
	\begin{subequations}
		\begin{align}
			\alpha &= \mathrm{LFC}(k_{\mathrm{init}} / k_{\mathrm{deg}}) \label{eq:alpha_def}\\
			\beta &= \mathrm{LFC}(k_{\mathrm{elong}} / k_{\mathrm{deg}}) \label{eq:beta_def}\\
			\gamma &= \mathrm{LFC}(k_{\mathrm{splice}} / k_{\mathrm{deg}}) \label{eq:gamma_def}
		\end{align}
	\end{subequations}
	The remaining three LFCs are obtained as their differences,
	$\mathrm{LFC}(k_{\mathrm{init}} / k_{\mathrm{elong}}) = \alpha - \beta$,
	$\mathrm{LFC}(k_{\mathrm{init}} / k_{\mathrm{splice}}) = \alpha - \gamma$ and
	$\mathrm{LFC}(k_{\mathrm{elong}} / k_{\mathrm{splice}}) = \beta - \gamma$, and reversing the
	order of the rates in a ratio flips the sign of its LFC.

	This parametrization corresponds directly to the steady state solution of the section \nameref{subsec:steady_state}. The mature transcript pool \eqref{eq:t_mat_ss} changes by $\mathrm{LFC}(T_{\mathrm{mature}}) = \alpha$, which is the quantity reported by a standard differential expression analysis; the nascent intron pool \eqref{eq:i_nas_ss} changes by $\alpha - \beta$, and the unspliced intron pool \eqref{eq:i_unspliced_ss} by $\alpha - \gamma$. The remaining combination $\beta - \gamma$ is the LFC of the ratio $k_{\mathrm{elong}} / k_{\mathrm{splice}}$, which determines the intron coverage shape through \eqref{eq:pi_rates}.
