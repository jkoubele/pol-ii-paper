...
\begin{document}
   ...
   \end{itemize}
	...
	We assume dataset consisting of $n$ samples. The samples are annotated by a design matrix $X \in \mathbb{R}^{n \times q}$, where $q$ is the number of features. The intercept column is not supposed to be a part of the design matrix, as out model handles intercept terms as explicit parameters.
	
	We will now describe a model fitted on a specific gene, with $m \geq 1$ introns. We will discuss a variants of this model, including a one fitted on data from multiple genes jointly later on. 
	...
	rates, and thus six such ratios. The corresponding six LFCs are, however, not independent of
	each other, as their LFCs span a space of dimension $3$. Thats is, our inference model needs to parametrize only three of them, with the remaining three LFCs following as their linear combinations.
