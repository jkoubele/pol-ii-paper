...
\begin{document}
   ...
   \end{itemize}
	...
	The model predicts several quantities, which give rise to the likelihood function, defined in the section \nameref{subsec:likelihood}. The likelihood function is then used for parameter estimation and statistical inference, as described below. Each of the predicted quantities is a vector in $\mathbb{R}^{n}$, holding one value per sample:
	\begin{itemize}
	...
	We will now describe the likelihood function of our model. The likelihood relates the quantities predicted in the section \nameref{subsec:model_param} to the data observed in an experiment, and consists of one term for each of the predicted quantities.
	...
	\begin{itemize}
		...
		\item The intron coverage $\boldsymbol{Y}^{\mathrm{cov}}_{i} \in \mathbb{N}_0^{n \times b}$ for each intron $i$, holding the number of reads falling into each bin, for every sample.
	\end{itemize}
	...
	Every intronic read falls into exactly one bin of its intron, so the row sums of the coverage are the intron read counts,
	\begin{equation} \label{eq:coverage_row_sums}
		\boldsymbol{y}^{\mathrm{intron}}_{i} = \boldsymbol{Y}^{\mathrm{cov}}_{i} \boldsymbol{1}
	\end{equation}
	where $\boldsymbol{1} \in \mathbb{R}^{b}$ is a vector of ones. The intron read counts and the intron coverage are therefore not two independent observations, and we will treat the coverage as the distribution of the reads within the intron, conditional on their total number.

	The exonic and intronic reads, on the other hand, are disjoint by construction, and we likewise require the introns not to overlap each other. Their read counts are therefore independent, and their contributions to the likelihood may be added.
