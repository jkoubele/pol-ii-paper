...
% the alignment midpoint.
	...
	The log-likelihood \eqref{eq:loglik_total} has no maximum in a closed form, and we therefore maximize it numerically. It is differentiable in all model parameters, so both the gradients used by the optimizer and the second derivatives used for the Fisher information below are obtained by automatic differentiation \citep{anselPyTorch2Faster2024}. The log-likelihood is not concave, and the optimization is for this reason not guaranteed to reach the global maximum. The value attained by the optimizer enters not only the estimates themselves, but also the test statistics described below, as those are computed from the maximized log-likelihood.

	\begin{itemize}
		\item We fit parameters by MLE.
		\item Loss is not convex, we use numerical method (L-BFGS in our implementation), but unfortunately without any guarantees (this may influence both estimated value and test statistics).
		\item Separation of data problem, like in logistic regression. Will cite \cite{hauckWaldsTestApplied1977}. Probably should be introduced here, after MLE but before testing. 
		\item  We may do Wald (cheap, just get loss Hessian), but due to separation of data, some params may not be identifiable. LRT circumvent this problem, and is therefore a default test in our implementation. QR on Hessian also produce identifiability flag.
		
		\item Also due to separation issue, we may use regularization (e.g., L2 regularization, a common/straightforward way to handle separation problem). So we use L2 (also it's simple / convenient). To get some priors (i.e. regularization strength), we pool estimates across genes and fit them via ashr method, using the weights from Wald (comment that this may have some shortcomings maybe, like the estimates are quite noisy in the first place; so we may get relatively weak regularization as a results, but this is probably ok.) Forward note that other way to pool the data, and thus regularize, is the global model described later. Brief note that we regularize everything on one fit, so no guarantee what will happen with params (e.g., some params may even grow in magnitute, as model tries to compensate for shrinkage of other params).
