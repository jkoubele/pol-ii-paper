...
\begin{document}
   ...
   \end{itemize}
	...
	\begin{itemize}
		\item All the operations above are applied elementwise, so that each equation holds separately in every sample. The intercept terms $c^{\mathrm{exon}}$, $c^{\mathrm{intron}}_{i}$ and $c^{\pi}_{i}$ are scalars, added to every entry of the corresponding vector.
		\item The predicted intronic reads are split into the two pools of the mixture \eqref{eq:mixture}: reads from nascent introns \eqref{eq:pred_mu_nascent}, and reads from unspliced introns \eqref{eq:pred_mu_unspliced}. Their changes between conditions are $\boldsymbol{\alpha} - \boldsymbol{\beta}_i$ and $\boldsymbol{\alpha} - \boldsymbol{\gamma}_i$, matching the steady state pools \eqref{eq:i_nas_ss} and \eqref{eq:i_unspliced_ss}.
		\item The prediction of $\boldsymbol{\pi}_i$ \eqref{eq:pred_pi} is the fraction of intronic reads originating from nascent introns, exactly as defined in \eqref{eq:pi_counts}. Dividing through by $\boldsymbol{\mu}^{\mathrm{nascent}}_{i}$ gives the equivalent form $\boldsymbol{\pi}_i = \sigma \left( c^{\pi}_{i} + \boldsymbol{X} (\boldsymbol{\gamma}_i - \boldsymbol{\beta}_i) \right)$, where $\sigma(x) = \exp(x) / (1 + \exp(x))$ is the sigmoid function.
