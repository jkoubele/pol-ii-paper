...
\begin{document}
   ...
   \end{itemize}
	...
	\begin{itemize}
		\item The division in \eqref{eq:pred_pi} is applied elementwise, and the scalar intercepts $c^{\mathrm{exon}}$, $c^{\mathrm{intron}}_{i}$ and $c^{\pi}_{i}$ are added to every entry of the corresponding vector.
		...
		\item The intercept $c^{\pi}_{i}$ therefore collects everything that shifts the coverage shape of the intron $i$ but does not change between the conditions, namely $\log ( L_{\mathrm{intron},i} / 2 )$ and the ratio $k_{\mathrm{splice},i} / k_{\mathrm{elong},i}$ in the reference condition, following \eqref{eq:pi_rates}. The intron lengths are for this reason not needed as an input of our model.
	\end{itemize}
